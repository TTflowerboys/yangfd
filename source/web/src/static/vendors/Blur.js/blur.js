 ...
 * @license MIT
 * Modified by Levy.li to add adjustSize in options
 */
...
define(['jquery'], function ($) {
	$.fn.blurjs = function (options) {
		...
		options = $.extend({
			...
			draggable: false,
			debug: false,
			adjustSize: false, //支持source的背景设置了background-size的情形
		}, options);
		...
		return this.each(function () {
			...
			tempImg.onload = function () {
				...
				if(options.adjustSize) {
					size = ($source.width() / $glue.width() * 100) + '%';
					$glue.css('background-size', size);
				}
