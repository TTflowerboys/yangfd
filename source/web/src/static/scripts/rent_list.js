...
(function () {
    ...
    var onePageMinItemCount = Math.ceil($('#main').height() / $('#rentCard_template').attr('data-cell-height'))
    ...
    function updatePropertyCardMouseEnter() {
        $('.rentCard').mouseenter(function(event){
            $(event.delegateTarget).find('button.openRequirement').show()
        });

        $('.rentCard').mouseleave(function(event){
