$(function(){
    ...
    var $tenantChatListContent = $('#tenantChatListContent')
    var $tenantPlaceHolder = $('#tenantPlaceHolder')
    var $hostChatListContent = $('#hostChatListContent')
    var $hostPlaceHolder = $('#hostPlaceHolder')
    ...
    var chatTpl = {
        ...
        message : function(message,time){
            return '<span class="text">'+message+'</span><span class="msgtime"><i class="icon-clock"></i>'+window.project.formatTime(parseInt(time))+'</span>'
        },
        itemsNew : function(rent_id,rent_title,rent_ticket_id,rent_ticket_user_id,user_nickname,message,time){
            return '<div class="chatListItmes"><div class="title"><span class="name">'+team.getUserName(user_nickname)+'</span><a href="/property-to-rent/'+rent_id+'" target="_blank">'+rent_title+'</a></div><div class="info sent"><div class="message" data-id="'+rent_ticket_id+'" data-user_id="'+rent_ticket_user_id+'"><span class="text">'+message+'</span><span class="msgtime"><i class="icon-clock"></i>'+window.project.formatTime(parseInt(time))+'</span></div></div><a href="/user-chat/'+rent_ticket_user_id+'/details" class="reply" target="_blank">'+i18n('回复')+'</a></div>';
        }
    }
    ...
    function addNewItems(obj, placeholder,socketVal, options){
        $.betterPost('/api/1/rent_intention_ticket/' + socketVal.id)
            .done(function (data) {
