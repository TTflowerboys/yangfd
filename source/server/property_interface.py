...
def property_wechat_poster(property_id):
    ...
    def format_property(property):
        res = u'<div><i class="icon-rooms"></i><span>' + unicode(str(property.get('bedroom_count',0))) + _(u'室') + unicode(str(property.get('living_room_count',0))) + _(u'厅')
        if property.get('space',{}):
            res += u"{0:.0f}".format(float(property.get('space',{}).get('value',0)))
            res += unicode(currant_util.format_unit(property.get('space',{}).get('unit','')))
        elif property.get('building_area',{}):
            res += u"{0:.0f}".format(float(property.get('building_area',{}).get('value',0)))
