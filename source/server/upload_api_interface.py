...
from libfelix.f_log import debug, warning

f_app.dependency_register("pillow", race="python")


@f_api('/upload', params=dict(
    nolog=("data"),
    data=("file", True),
...
def upload(params):
    ...
    try:
        ...
        abort(40074, warning("Invalid params: Unknown image type.", exc_info=False))
    ...
    if params["thumbnail_size"][0] > 0:
        ...
        if thumbnail_height > original_width:
            abort(40000, warning('Invalid thumbnail size: cannot be larger than width param', exc_info=False))
...
@f_api('/upload_mime_html', params=dict(
    nolog=("data"),
    data=("file", True),
    thumbnail_size=(list, [600, 300], int),
    width_limit=(int, 1280),
