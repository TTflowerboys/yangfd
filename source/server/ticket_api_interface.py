...
def intention_ticket_add(params):
    ...
    title = template("static/emails/receive_intention_title")
    f_app.email.schedule(
        target=params["email"],
...
@f_api('/rent_intention_ticket/add', params=dict(
    ...
    male_roommates=int,
    female_roommates=int,
    minimum_rent_period="i18n:time_period",
    bedroom_count=(list, None, "enum:bedroom_count"),
    country="country",
    description=str,
    title=str,
    city="geonames_gazetteer:city",
...
def rent_intention_ticket_search(user, params):
    ...
    if "interested_rent_ticket_user_id" in params:
        ...
        params["interested_rent_tickets"] = lmap(ObjectId, f_app.ticket.search({"type": "rent", "user_id": params.pop("interested_rent_ticket_user_id")}, per_page=-1))
    ...
    if set(user_roles) & set(["admin", "jr_admin", "sales", "operation", "jr_operation"]):
        ...
        enable_custom_fields = False
        if force_user_id and ("user_id" not in params or user['id'] != f_app.user.get(params['user_id']).get('referral')):
            params["user_id"] = ObjectId(user["id"])
    ...
    if len(property_params):
        ...
        rent_ticket_id_list = lmap(ObjectId, f_app.ticket.search(rent_ticket_params, per_page=0))
        params["interested_rent_tickets"] = {"$in": rent_ticket_id_list}

    if len(user_params):
        user_params.setdefault("status", {"$ne": "deleted"})
        user_id_list = lmap(ObjectId, f_app.user.search(user_params))
    ...
    if "status" in params:
        if set(params["status"]) <= set(f_app.common.rent_intention_ticket_statuses):
            ...
            abort(40093, logger.warning("Invalid params: status", params["status"], exc_info=False))
    return f_app.ticket.output(f_app.ticket.search(params=params, per_page=per_page, sort=sort, time_field=sort[0]), enable_custom_fields=enable_custom_fields)
...
def rent_intention_ticket_sms_send(rent_intention_ticket_id, user, params):
    ...
    sms = {
        "method": "nexmo",
        "sender": nexmo_number["phone"].lstrip("+"),
        "target": user["phone"],
        "text": params["text"],
    }
...
@f_app.user.login.check(force=True, check_role=True)
def rent_intention_ticket_chat_history(rent_intention_ticket_id, user, params):
    ticket = f_app.ticket.get(rent_intention_ticket_id)
    assert ticket["type"] == "rent_intention", abort(40000, "Invalid rent_intention ticket")
    if "user_id" in params and str(params["user_id"]) != user["id"]:
...
@f_api('/rent_ticket/add', params=dict(
    ...
    rent_deadline_time=datetime,
    minimum_rent_period="i18n:time_period",
    holding_deposit="i18n:currency",
    deposit="i18n:currency",
    landlord_type="enum:landlord_type",
    bill_covered=bool,
    independent_bathroom=bool,
    current_male_roommates=int,
    current_female_roommates=int,
    accommodates=int,
...
def _rent_ticket_search(user, params):
    ...
    if "hesa_university" in params:
        property_params["featured_facility.hesa_university"] = params.pop("hesa_university")

    if "doogal_station" in params:
        property_params["featured_facility.doogal_station"] = params.pop("doogal_station")
    ...
    if len(property_params):
        ...
        if "latitude" in property_params:
            ...
            property_id_list = lmap(ObjectId, f_app.property.get_nearby(property_params, output=False))
        else:
            property_id_list = lmap(ObjectId, f_app.property.search(property_params, per_page=0))
...
def rent_ticket_suspend(ticket_id, user):
    ...
    if ticket_email_user:
        title = template("static/emails/rent_suspend_notice_title")
        f_app.email.schedule(
